...
QString QmlBackend::getPublicServerList() {
  ...
  if (!conf.exists()) {
    if (!conf.open(QIODevice::WriteOnly)) {
      qWarning() << "Failed to create server-list.json";
      return "[]";
    }
  ...
  if (!conf.open(QIODevice::ReadOnly)) {
    qWarning() << "Failed to open server-list.json for reading";
    return "[]";
  }
...
QString QmlBackend::loadTips() {
  ...
  if (!conf.exists()) {
    if (!conf.open(QIODevice::WriteOnly)) {
      qWarning() << "Failed to create waiting_tips.txt";
      return "转啊~ 转啊~";
    }
  ...
  if (!conf.open(QIODevice::ReadOnly)) {
    qWarning() << "Failed to open waiting_tips.txt for reading";
    return "转啊~ 转啊~";
  }
  auto ret = conf.readAll();
  conf.close();
...
void QmlBackend::playSound(const QString &name, int index) {
  ...
  if (!fileInfo.exists()) {
    qWarning() << "Sound file not found:" << fname;
    return;
  }
  ...
  output->setVolume(m_volume / 100.0);

  // 使用绝对路径创建 URL
  QUrl sourceUrl = QUrl::fromLocalFile(absolutePath);
  ...
  connect(player, &QMediaPlayer::errorOccurred, player,
          [player, absolutePath](QMediaPlayer::Error error, const QString &errorString) {
    qWarning() << "Error playing sound:" << absolutePath;
